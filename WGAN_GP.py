...
#         real = torch.tensor(torch.rand(logit_real.size())*0.25 + 0.85)
        ...
        # here we don't count G, for G is not trained in WGAN yet
        G_loss = self.supervised_criterion(fake, fake)
        # Supervised Loss
    ...
    def training(self, dataloaders,
        ...
        Args:
            ...
            # of steps of training of Discriminator alone at first. By default, we set the value to be 10000.
            first_steps (int) -
            # of steps of pretraining of Generator. It should be equal to the actual pretrained steps (250000 here).
            num_steps_pre (int) -
    ...
    #             self.optimizerD.zero_grad()
    #             with torch.set_grad_enabled(False):
    #                 sr_patches, _, _, _ = self.forwardDG(
    #                     lr_patches, hr_patches)
    #                 # statistics
    #                 # concatenate patches, send patches to cpu to save GPU memory
    #                 sr_data_cat = torch.cat(
